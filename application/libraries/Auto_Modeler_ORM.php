...
class Auto_Modeler_ORM extends Auto_Modeler
{
	...
	public function __get($key)
	{
		...
		if (isset($this->data[$key.'_id']))
		{
			...
			return $this->db->from(inflector::plural($key))->where('id', $this->data[$key.'_id'])->get()->result(TRUE, ucwords($key).'_Model')->current();
	...
	public function find_parent($key = NULL)
	{
		...
		{
			return $this->db->from(inflector::plural($key))->where('id', $this->data[$key.'_id'])->get()->result(TRUE, ucwords($key).'_Model');
		}
		else
		{
